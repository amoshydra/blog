...
Clips from two Google TTS builds on Android, the later one a marked, unverified draft, let you [hear what the voice actually does](#the-clips-and-their-transcripts).
...
Android's API returns audio and never the normalized string, so the words have to come from listening. The gallery below carries a clip and a transcript for each amount. The `103.12.8` column is verified against the audio; the `20260817` column is a draft capture, unverified and hidden behind a toggle. Currency, the number, the point word and the minor unit are each coloured, with a second, non-colour cue so the split survives without colour; non-Latin read-outs carry a romanization underneath.
