...
Clips from two Google TTS builds on Android and an eSpeak NG column let you [hear what the voice actually does](#the-clips-and-their-transcripts); the later Google build is a marked, unverified draft, and eSpeak appears as phonemes rather than words.
...
Android's API returns audio and never the normalized string, so the words have to come from listening. The gallery below carries a clip and a transcript for each amount. The `103.12.8` column is verified against the audio; the `20260817` column is a draft capture, unverified and hidden behind a toggle; the `eSpeak NG` column is an open engine shown as phonemes, also behind a toggle. Currency, the number, the point word and the minor unit are each coloured, with a second, non-colour cue so the split survives without colour; non-Latin read-outs carry a romanization underneath.
...
To check any of these readings yourself, the picker used here lives in the repository at [`tools/transcript-verifier/`](https://github.com/amoshydra/blog/tree/main/tools/transcript-verifier). It runs locally from a clone, plays each clip, and offers the draft, a few candidate read-outs, or your own text. It writes your answer into `transcripts.json`, which is what promotes a line from draft to verified.
