...
With a Microsoft voice selected, NVDA reads the first as *"one hundred and twenty-three US dollars and forty-five cents"*. It reads the second as a spelled-out code and then a decimal number: *"S G D, one hundred and twenty-three point four five"*. No "Singapore dollars", no cents. Narrator, Microsoft's own screen reader, produces the same two readings [^16]. That observation comes from Windows with a Microsoft voice; the broader per-reader results later in the post come from a published comparison, and I did not re-run them. The markup is the same in both readings, and so is the browser.

The digits come through both times, so the value is never wrong. What `SGD` loses is the words: the listener hears a code they may have to decode, and never hears the currency's name or its minor unit. On a page that mixes Singapore and US amounts, that is work the listener would not otherwise have to do.

NVDA ships with eSpeak NG, which does not expand currency, so the reading above came from a Microsoft engine rather than from NVDA itself [^2].
...
What differs is the currency list, which is short and rarely published. `$` and `USD` sit in the en-US table and `SGD` does not [^6] [^16]; NVDA and TalkBack ship no table, and the open normalizers from Google and NVIDIA ship small ones [^24] [^25]. HTML has no currency element [^27], so write the currency name out or supply the spoken form yourself.
