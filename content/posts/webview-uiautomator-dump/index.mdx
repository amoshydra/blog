...
<HtmlXmlCompare
  label={"HTML `id`"}
  note={"maps to <code>resource-id</code> on current builds (WebView 115, Chrome 151/153, Vanadium 152 — all verified); note it was dropped in the Chrome 122 era, see Gotcha 3"}
...
### 3. `id` → `resource-id` is version-dependent

The HTML `id` attribute surfaces as the node's `resource-id` — verified live here on **four** engines: WebView 115 (Google Android System WebView, the OnePlus 6), Chrome **151** (an embedded WebView, no accessibility service), Chrome **153** (the browser, on this post's second test device), and Vanadium WebView **152** (same device).

But it *was* dropped: **WebView/Chrome 122 (2024)** stopped exporting it, famously breaking automation that located web elements by `resource-id` (upstream issue in the appendix) — people factory-reset to WebView 117 to get the ids back. Current builds (2026) export it again; the removal reads as a regression that was later reversed, not a permanent contract.

The practical consequence is that `resource-id` from web content is **not a stable contract across versions**. It works on today's builds, but a locator built on it can break when a device's WebView updates — so it is not the handle to reach for. The stable, version-independent handle in the a11y tree is **role + accessible name** — the `text`/`content-desc` a screen reader announces. That is the best practice on both sides:
...
|---|---|---|
| A11y tree (`uiautomator dump` / `NATIVE_APP`) | role + accessible name (`text` / `content-desc`) | `resource-id` from HTML `id` (regressed once, in the 122 era — version-flaky); `data-testid` |
...
- **Target by accessible name** (`text` / `content-desc`) and role-derived classes — not `resource-id` (version-dependent) and never `data-testid`.
...
- [Chromium issue #328681381](https://issues.chromium.org/328681381) — WebView 122 removed HTML-`id`→`resource-id` export, breaking Appium locators.
