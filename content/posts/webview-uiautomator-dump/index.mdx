...
- **The WebView's part of that tree is built lazily** — when no accessibility service is *bound*. After a cold start, the *first* dump shows only a bare `<node class="android.webkit.WebView">`; the *second* dump has your whole page. This is why "content isn't in the XML until I run the dump again" happens — and why a modal or late-appearing component seems to "sometimes" vanish. A bound accessibility service (a real screen-reader client) or `--force-renderer-accessibility` — a WebView command-line flag (see Gotcha 1) — makes the first dump complete.
...
- [Open the HTML — the full gallery page](artifacts/dump.html)
- [Open the XML — the full warm dump](artifacts/dump.xml)

Then the same dump, broken out one element at a time — HTML on the left, its dumped node on the right. All the fixture's static cases appear below (the two dynamic ones — delayed content and the scroll region — are covered in Gotchas 1 and 2; the page headings around each fixture case aren't cases themselves). The last three — cases 32–34, the `sr-only`/`aria-hidden` comparison — come from a separate screen-reader-enabled capture — they aren't in the warm `dump.xml`; the worked example below walks through them.
...
<HtmlXmlCompare
  label={"Heading"}
  note={"the heading role is not exposed by the dump: there is no <code>heading</code> attribute in its fixed attribute set (Appium can read a <code>heading</code> attribute on API 28+ where the platform sets it)"}
  html={"<h3>\n  Case Two Heading\n</h3>"}
...
- **Roles are lossy.** Control roles become platform classes and flags (`Button`, `CheckBox`, `EditText`, `SeekBar`, `Image`, `RadioButton`). Roles the dumper has no attribute for are not exposed as such — `heading` and `dialog` produce no distinguishing `class` or attribute in the XML (Appium can surface a `heading` attribute on API 28+ where the platform sets it; the dump itself cannot).
...
- **A *bound* accessibility service does fix it.** On a re-visit of this device with TalkBack enabled and *bound* (verified via `dumpsys accessibility` — don't trust the settings toggle alone: a failed `settings put` once left our control reading identical), the first cold dump was complete, whether TalkBack was bound *before* or *enabled after* page load — a live client keeps the renderer's accessibility on from content load. The original study capture of "TalkBack on, first dump still 8" was evidently taken while the service was still binding (`dumpsys` shows `Bound services:{}` for the *empty* state; the setting alone can lie for a few seconds). What has no effect is weak-a11y modes that don't keep a client connected.
- Without a bound service, the lazy tree is per-WebView-and-per-connection: every fresh connection to a fresh WebView costs one incomplete first dump.
...
- Open → close → reopen → each reflected on the very next dump (re-verified 12/12 in a later session, including a +0.7 s turn-around, with and without an accessibility service bound).
...
| | `NATIVE_APP` (accessibility tree) | `WEBVIEW` (DOM over CDP, the Chrome DevTools Protocol) |
|---|---|---|
| Source | The same `AccessibilityNodeInfo` tree `uiautomator dump` reads | The real HTML DOM, served over the WebView's debugging socket via CDP (chromedriver) |
...
For a WebView-heavy app, the two engines fit different jobs. The DOM/CDP path is the natural tool for *functional* coverage of the web layer — real selectors and `data-testid` — which is why hybrid-mode automation drives WebViews through chromedriver. Its catch is the socket: it exists only on a debug-enabled build or a userdebug device, so it can't validate the production artifact. The a11y tree is the opposite: lossy, but present on every build, release included. The best-practice split is to use both — DOM/CDP for detailed functional tests on a debug-enabled build or release-signed QA flavour, and the a11y tree for the production sign-off and accessibility checks.
...
Is this a bad pattern? First, notice that *"Contact James Roger has 23 unread emails"* is not the same utterance as the four pieces: the sentence carries the meaning — the relationship, the count — while the pieces are a fragmented visual rendering. So the `sr-only` sentence is not duplicating the visible content; it is the coherent statement the visual design otherwise hides. If that statement is the whole story, `sr-only` + `aria-hidden` is a legitimate pattern, and automation can still target the sentence — it is in the tree, as the dump above shows. The pattern only becomes a problem when an individual value is itself data someone needs to address — a count, a name, a link — because `aria-hidden` removes it for real users too, not just for automation. There is no separate "automation tree" to compromise: `uiautomator dump` reads the same tree TalkBack reads, so the decision is never "users vs. tests" — it is what belongs in the tree, decided for users.

Can you have both — the clean announcement *and* individually reachable values? We tested the candidates with TalkBack, and in this WebView the answer is no:

- **`aria-label` on the `<dl>` itself** — not announced at all: `<dl>` isn't nameable, so TalkBack skips the label.
- **`role="group" aria-label` wrapping the `<dl>`** — the tree carries the sentence on the group node and the values as discrete nodes, but TalkBack does not speak the group's label. It reads only the `<dl>` values, with the structure: `Contact` *term*, `James Roger` *definition*, `Emails` *term*, `23` *definition*. The constructed line is never said.
- **`sr-only` sentence next to a visible `<dl>`** — the sentence *is* spoken, but then the values are read too, so it is not one clean announcement.

The reason is structural: the accessibility tree is the single source for both TalkBack and automation. Anything in it can be announced, and TalkBack does not reliably speak container labels (groups, definition lists) during normal navigation. No pattern we tested — and, as far as we can tell structurally, no HTML pattern — announces the constructed line *and* keeps the values silent-but-reachable.

The practical conclusion is to keep the `sr-only` + `aria-hidden` pattern when the clean announcement is the goal — it is the one pattern that actually delivers it, and automation can still target the sentence, because the sentence *is* in the tree (the dump above shows it). If a specific value must be individually reachable, that conflicts with the clean announcement; decide which matters and accept the other is lost. Do not restructure the markup to satisfy a locator — the values are hidden by design, and that is a screen-reader decision, not an automation bug.

**Rule of thumb:** an `sr-only` sentence is the reliable way to announce a constructed line, and `aria-hidden` removes content from the tree for everyone — users and automation alike. If a value must be individually reachable, it cannot also be hidden for a clean announcement. Pick one, and don't let the test decide.
...
Environment: any device with a WebView launcher app and `adb` (this study used the author's [simple WebView launcher](https://github.com/amoshydra/android-webview-launcher), which just loads a URL you pass it), or the same fixture over your LAN. All facts above were captured against a OnePlus 6 (LineageOS 20, `ro.build.type=userdebug`), WebView `com.android.webview` 115.0.5790.136, Android 13, screen 1080×2280 with a density override of 314. The `id`→`resource-id` and lazy-tree findings were re-verified on a second device — a Pixel 7 Pro (`ro.build.type=user`), where Vanadium WebView 152 and Chrome 153 both export `resource-id` (and where the `--force-renderer-accessibility` flag is ignored).
...
The full fixture (`gallery.html`) contains 34 cases, including the `sr-only`/`aria-hidden` comparison at the end (cases 32–34, the ones the worked example above uses), the JS modal (`?auto=modal`), delayed content (`?auto=delay`), `id`/`data-testid`, roles, form controls, and a long scroll region. The fixture and the raw dumps are published with this post: [gallery.html](artifacts/gallery.html), [the rendered tree](artifacts/dump.html), and [the raw warm dump](artifacts/dump.xml).

### Appendix: the three accessibility states

| State | First dump after cold start |
|---|---|
| Baseline (`accessibility_enabled=0`, no flag, no client) | 8 nodes, bare `WebView` |
| Screen reader on (TalkBack **bound** — verified via `dumpsys accessibility`) | **full page** — a live client keeps the renderer's accessibility on |
...
- [Chromium issue #328681381](https://issues.chromium.org/328681381) — WebView builds in the ~120–126 range stopped exporting HTML-`id`→`resource-id`, breaking Appium locators (first widely reported against 122); **marked Fixed** (see next).
