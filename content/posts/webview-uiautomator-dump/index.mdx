...
The reason is structural: the accessibility tree is the single source for both TalkBack and automation. Anything in it can be announced, and TalkBack does not reliably speak container labels (groups, definition lists) during normal navigation. There is no HTML pattern that announces the constructed line *and* keeps the values silent-but-reachable.
